...
# This migration is mostly the same as realmauditlog_extra_data_json
...
def do_bulk_backfill_extra_data(
    ...
    # First handle the special case for audit logs with the USER_FULL_NAME_CHANGED event, which stores
    # the full name not as str(dict()) but a plain str.
    # Note that we only update the entries where extra_data_json has the default value,
    # because we do not want to override existing audit log entries with a NEW_VALUE of None for extra_data_json.
    # We do not need to skip existing entries for other parts of backfilling because we have double-write implemented
    # so that the backfilled value will still be consistent.
    audit_log_model.objects.filter(
        event_type=USER_FULL_NAME_CHANGED,
        id__range=(id_lower_bound, id_upper_bound),
        extra_data_json={},
        # extra_data used to keeps track of the old name. As a result, we know nothing about
        # what NEW_VALUE would be especially if the name has been changed multiple times.
        # extra_data_json is a JSONObject whose OLD_VALUE and NEW_VALUE is mapped from the value
        # of the extra_data field (which is just a old full name string) and None, respectively.
    ...
    for audit_log_entry in python_valued_audit_log_entries:
        ...
        # str()'d extra_data with the REALM_DISCOUNT_CHANGED event type is not handled by this
        # migration. We expect that all such entries are manually converted beforehand or an
        # error will occur during the migration, because ast.literal_eval does not allow the
        # evaluation of Decimal.
        ...
        if audit_log_entry.event_type == REALM_DISCOUNT_CHANGED:  # type: ignore[attr-defined] # Explained above.
            print(
                f"Audit log entry {audit_log_entry.id} with event type REALM_DISCOUNT_CHANGED is skipped. The data consistency needs to be manually checked:\n"  # type: ignore[attr-defined] # Explained above.
                f"  The discount data that will be removed after the upcoming JSONField migration: {audit_log_entry.extra_data}\n"
                f"  The discount data that will be kept after the upcoming JSONField migration: {old_value}"
            )
            continue
